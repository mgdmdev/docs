...
<Steps>
  <Step title="Redirect to authorize">
    ...
    ```
    GET https://auth.mgpass.net/authorize?
      response_type=code
  ...
  <Step title="Exchange code for tokens">
    Exchange the authorization code for tokens. **Confidential clients (web /
    m2m) must authenticate** — send `client_secret` in the body (shown below) or
    via HTTP Basic (`Authorization: Basic base64(client_id:client_secret)`). A
    missing or wrong secret returns `401 invalid_client`. Public clients use PKCE
    instead (next section) and send no secret.
    ...
    const response = await fetch("https://auth.mgpass.net/token", {
      method: "POST",
