...
description: "Configure and use SMS one-time passwords for user verification in mgPass."
---

## Overview

mgPass supports SMS-based one-time password (OTP) verification. This is used for phone number verification, passwordless sign-in, and as a second factor in MFA flows.

## How OTP Works

<Steps>
  <Step title="Request OTP">
    The application requests an OTP for a phone number. mgPass generates a 6-digit code and sends it via SMS.
  </Step>
...
Configure the SMS connector in the mgPass developer console:

| Field | Description |
|-------|-------------|
| `api_key` | SMS provider API key |
| `client_id` | SMS provider client ID |
