...
<AccordionGroup>
  ...
  <Accordion title="Register post-logout URIs">
    Configure your application's `post_logout_redirect_uris` in the mgPass admin console. At logout, mgPass identifies your app from the `id_token_hint` **or** a `client_id` query parameter, and only redirects to a `post_logout_redirect_uri` that **exactly matches** a registered value (trailing-slash differences are tolerated). An unregistered or unidentified URI returns a plain "logged out" response instead of redirecting — this prevents open-redirect abuse.
  </Accordion>
</AccordionGroup>
