...
- `console.mgpass.net` -- developer console

When a user authenticates, the mgPass authorization service sets a `mgpass_session` cookie on the `.mgpass.net` domain. Because all mgPass properties are subdomains, the cookie is sent with every request automatically.

### How It Works

<Steps>
  <Step title="User signs in">
    The user authenticates on any mgPass surface (e.g. the account portal at `mgpass.net`). The mgPass authorization service sets the `mgpass_session` cookie on `.mgpass.net`.
  </Step>

  <Step title="User visits another mgPass property">
    The user navigates to the developer console at `console.mgpass.net`. The session guard detects no local session.
  </Step>

  <Step title="Silent authorization">
    The session guard auto-redirects to `auth.mgpass.net/authorize?prompt=none`. Because the browser sends the shared `.mgpass.net` cookie, the mgPass authorization service finds the active session.
  </Step>

  <Step title="Code issued silently">
    The mgPass authorization service issues an authorization code and redirects back to `console.mgpass.net`. No login UI is shown.
  </Step>

  <Step title="Token exchange">
    The developer console exchanges the authorization code for tokens. The user is logged in instantly.
  </Step>
</Steps>
...
sequenceDiagram
    participant User
    participant Console as console.mgpass.net
    participant Auth as auth.mgpass.net

    User->>Console: Visit console.mgpass.net
    Console->>Console: No local session found
    Console->>Auth: Redirect: /authorize?prompt=none
    Note over Auth: Browser sends mgpass_session<br/>cookie (shared on .mgpass.net)
    Auth->>Auth: Validate session cookie against session cache
    Auth-->>Console: Redirect with authorization code
    Console->>Auth: Exchange code for tokens
    Auth-->>Console: Access token + ID token
    Console-->>User: Logged in (no UI shown)
```
...
When a user logs out, mgPass ends their shared sign-on session for that browser — so they must authenticate again to reach any mgPass-connected app — and revokes the offline access for the app they logged out from.

### What happens

<Steps>
  <Step title="User triggers logout">
    The user clicks "Log out" in your app (or any mgPass-connected app).
  </Step>

  <Step title="Redirect to the logout endpoint">
    Your app redirects to `auth.mgpass.net/logout` with an `id_token_hint` (or a `client_id`) and a `post_logout_redirect_uri`.
  </Step>

  <Step title="Sign-on session ended">
    mgPass clears the shared `mgpass_session` cookie and revokes that browser's sign-on session, so silent SSO no longer succeeds until the user signs in again.
  </Step>

  <Step title="App tokens revoked">
    Refresh tokens issued to the requesting app are revoked, ending its offline access. Other apps' offline access is left intact.
  </Step>

  <Step title="First-party surfaces notified">
    mgPass signals its own surfaces (such as the account portal) to clear the user's local session.
  </Step>

  <Step title="User redirected">
    If the `post_logout_redirect_uri` exactly matches one registered for the app, the user is redirected there; otherwise a plain logged-out response is returned.
  </Step>
</Steps>
...
sequenceDiagram
    ...
    App->>Auth: Redirect: /logout?id_token_hint=...&post_logout_redirect_uri=...
    Auth->>Auth: Clear mgpass_session cookie
    Auth->>DB: Revoke this browser's session
    Auth->>DB: Revoke the app's refresh tokens
    Auth->>Account: Back-channel: clear local session
    Auth-->>User: Redirect to registered post_logout_redirect_uri
```
...
<AccordionGroup>
  ...
  <Accordion title="Handle all callback states">
    ...
    Consent is recorded durably on first authorization, so `consent_required`
    only fires the **first** time a user authorizes your app — subsequent SSO is
    silent. This holds even for apps that don't request `offline_access`.
  </Accordion>
  ...
  <Accordion title="Register post-logout URIs">
    Configure your application's `post_logout_redirect_uris` in the mgPass developer console. At logout, mgPass identifies your app from the `id_token_hint` **or** a `client_id` query parameter, and only redirects to a `post_logout_redirect_uri` that **exactly matches** a registered value (trailing-slash differences are tolerated). An unregistered or unidentified URI returns a plain "logged out" response instead of redirecting — this prevents open-redirect abuse.
  </Accordion>
</AccordionGroup>
