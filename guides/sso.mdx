...
description: "How mgPass provides seamless single sign-on across all MG Digital properties and partner applications."
---

## Overview

mgPass provides Single Sign-On (SSO) so users authenticate once and are automatically recognized across all connected applications. Whether a user signs in on 3News and then visits adesa+, or logs into the admin console after authenticating on account.mgpass.net, mgPass handles the session sharing transparently.
...
- `account.mgpass.net` -- user account portal
...
Applications on different domains (3News, adesa+, partner sites) cannot share cookies with `.mgpass.net`. Instead, they use the OIDC `prompt=none` redirect pattern to check for an active session.
...
function buildSilentAuthURL() {
  ...
  return `https://pass.mediageneral.digital/oidc/auth?${params}`;
}
...
const MGPASS_AUTH = "https://pass.mediageneral.digital/oidc/auth";
...
sequenceDiagram
    participant User
    participant App as 3news.com
    participant Auth as auth.mgpass.net

    User->>App: Visit 3news.com
...
1. **User triggers logout** from any application (3News, adesa+, admin console, account portal)
...
```
GET https://pass.mediageneral.digital/logout?
...
function logout(idToken) {
  ...
  window.location.href =
    `https://pass.mediageneral.digital/logout?${params}`;
}
```
...
def get_logout_url(id_token: str, redirect_uri: str) -> str:
    ...
    return f"https://pass.mediageneral.digital/logout?{params}"
```

```bash cURL
# RP-Initiated Logout (browser redirect, not a REST call)
# Redirect the user's browser to:
https://pass.mediageneral.digital/logout?\
...
sequenceDiagram
    ...
    participant Auth as auth.mgpass.net
    participant Admin as admin.mgpass.net
    participant Account as account.mgpass.net
