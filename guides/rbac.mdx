...
- **API Resource** = "Streaming API" (the thing being protected)
...
| app:subscriber | stream:live, stream:vod, profile:read | Paid subscribers |
| app:premium | stream:live, stream:vod, content:download, profile:read | Premium subscribers |
| tickets:organizer | events:create, events:read, tickets:manage | Event organizers |
| mgpass:admin | Full admin access | Platform administrators |
...
## Setting Up RBAC (Developer Console)

### Step 1: Create an API Resource

<Steps>
  <Step title="Navigate to Roles > API Resources">
    In the developer console, go to **Roles** in the sidebar, then click the **API Resources** tab at the top.
  </Step>
  <Step title="Create the resource">
    Fill in:
    - **Name** — human-readable label (e.g., "Streaming API")
    - **Identifier** — unique URI used as the `audience` in JWTs (e.g., `https://api.yourapp.com`)
...
<Steps>
  ...
  <Step title="Create a new role">
    ...
    - **Name** — unique identifier (e.g., `app:subscriber`)
...
```bash
curl -X POST https://auth.mgpass.net/api/api-resources/res_abc123/scopes \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "stream:live",
...
curl -X POST https://auth.mgpass.net/api/roles \
  ...
  -d '{
    "name": "app:subscriber",
    "description": "Subscriber with streaming access",
    "type": "user",
    "is_default": false
  }'
```
...
```bash
curl -X POST https://auth.mgpass.net/api/roles/role_abc123/scopes \
  -H "Authorization: Bearer ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE https://auth.mgpass.net/api/users/usr_abc123/roles/role_subscriber \
  -H "Authorization: Bearer ADMIN_TOKEN"
```
...
```json
{
  ...
  "roles": ["app:subscriber"],
  "iss": "https://auth.mgpass.net",
  "iat": 1711900000,
  "exp": 1711903600
}
```
...
<Note>
Default roles only apply to new registrations. Existing users without the role are not affected. To backfill, assign the role via the API or developer console.
</Note>
...
## Real-World Example: Streaming App Integration

Here's how the full RBAC setup works for a streaming application:

<Steps>
  <Step title="Create API Resource">
    Name: "Streaming API", Identifier: `https://api.yourapp.com`
    Scopes: `stream:live`, `stream:vod`, `content:download`, `profile:read`
  </Step>
  <Step title="Create Roles">
    - **app:free** — `stream:live` (limited), `profile:read`
    - **app:subscriber** — `stream:live`, `stream:vod`, `profile:read`
    - **app:premium** — all scopes including `content:download`
  </Step>
  <Step title="Register Your App as an Application">
    Type: Native (mobile) / SPA (web). Allowed scopes: all streaming scopes.
  </Step>
  ...
  <Step title="Check Scopes in Your API">
    Your backend validates the JWT and checks for required scopes before granting access to streams.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Use Meaningful Names" icon="tag">
    `app:subscriber` is better than `role_1`. Names appear in admin UI and audit logs.
  </Card>
