...
sequenceDiagram
    ...
    App->>Auth: POST /token (refresh_token grant)
...
class TokenManager {
  ...
  async trySilentAuth() {
    ...
    window.location.href =
      `https://auth.mgpass.net/authorize?${params}`;
  }
}
```
...
<Warning>
Never store multiple copies of a refresh token or retry with an old token after receiving a new one. Genuine reuse of an old refresh token triggers replay detection and locks out the user.
</Warning>

<Note>
**Concurrent refreshes are safe.** Two requests that race the *same* refresh
token (e.g. multiple tabs) won't trip replay detection: one wins and rotates,
the other gets `409 invalid_grant` (`rotation_in_progress`) or a plain
`invalid_grant` within a short grace window — the session is **not** revoked.
Replay detection (revoke-all) only fires for genuine reuse outside that window.
You still shouldn't deliberately reuse an old token; just don't fear a momentary
race.
</Note>
...
flowchart TD
    ...
    B -- Yes --> C[POST /token with refresh_token]
...
sequenceDiagram
    ...
    BFF->>Auth: POST /token (refresh_token from cookie)
    Auth-->>BFF: New tokens
...
Token lifetimes are set per application in the **developer console**, under your application's OAuth settings:
