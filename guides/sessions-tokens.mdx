...
sequenceDiagram
    ...
    mgPass->>DB: Create session record
    mgPass->>Cache: Cache session
    mgPass->>App: Issue tokens
    App->>mgPass: API request with access token
    mgPass->>Cache: Validate session (fast path)
    mgPass->>App: Response
```

### Durable + Cached Storage

Sessions are stored durably and cached for fast validation:

- **Session cache** — Primary lookup for token validation (fast path)
- **Database** — Durable record for session management and audit

## Token Types

| Token | Format | Lifetime | Purpose |
|-------|--------|----------|---------|
| **Access Token** | JWT (ES256) | 1 hour (configurable) | Authorize API requests |
| **Refresh Token** | Opaque string | 30 days (configurable) | Obtain new access tokens |
| **ID Token** | JWT (ES256) | 1 hour | User identity claims (OIDC) |
...
```json
{
  ...
  "email_verified": true,
  "picture": "https://cdn.mgpass.net/avatars/usr_abc123.jpg",
  "iat": 1711900000,
  "exp": 1711903600
}
```
...
# Revoke a session
curl -X POST https://auth.mgpass.net/api/account/sessions/sess_abc123/revoke \
  -H "Authorization: Bearer USER_TOKEN"
```
