...
Webhooks are configured in the [developer console](https://console.mgpass.net). You provide:
...
For [one-off payments](/guides/one-off-payments). `failed` and `abandoned` are separate
events because they are different things that happened to a buyer.

| Event | Description |
|-------|-------------|
| `payment.succeeded` | Money moved. Safe to fulfil. |
| `payment.failed` | The provider declined the charge |
| `payment.abandoned` | Nobody completed the charge before it expired |
...
A delivery is considered successful on any `2xx` response. After 4 failed attempts, the delivery is marked as failed and can be manually replayed from the developer console.
