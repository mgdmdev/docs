...
Every platform in your ecosystem is a registered application: your web apps, mobile apps, the admin console itself, and any third-party integrations.
...
<AccordionGroup>
  ...
  <Accordion title="Native / Mobile" icon="mobile">
    ...
    **Example:** Your mobile app or a native desktop client.
  </Accordion>
  ...
  <Accordion title="Machine-to-Machine (M2M)" icon="robot">
    ...
    **Example:** A rewards backend calling the mgPass Management API to look up user profiles. A ticketing service calling the rewards API to publish ticket purchase events.
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Click Create Application">
    ...
    - **Name** — displayed to users on the consent screen (e.g., "My App", "Partner Portal")
...
```bash
curl -X POST https://auth.mgpass.net/api/clients \
  ...
  -d '{
    ...
    "post_logout_uris": ["https://yourapp.com"],
...
```bash
curl -X POST https://auth.mgpass.net/api/clients/app_abc123/rotate-secret \
  -H "Authorization: Bearer ADMIN_TOKEN"
```
